...
export interface ReleaseInfo {
  releaseType: "official" | "unofficial" | "devel" | "Modded";
  version: string;
...
export async function listModdedReleases(): Promise<ReleaseInfo[]> {
  ...
  // TODO - even longer term - extract this out into an API we control (avoid github rate limiting) -- will be needed for unModded releases as well anyway
...
export async function getLatestModdedRelease(): Promise<ReleaseInfo> {
  // TODO - handle rate limiting
  // TODO - even longer term - extract this out into an API we control (avoid github rate limiting) -- will be needed for unModded releases as well anyway
  const resp = await fetch(
